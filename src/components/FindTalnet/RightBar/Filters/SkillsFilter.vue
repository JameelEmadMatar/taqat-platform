        ...
        <div class="select">
            <div class="selected-container">
                ...
                <input 
                    ...
                    class="search-input" 
                    v-model="inputValue" 
                    :placeholder="optionSelected.length === 0 ? 'Search skills...' : ''"
            ...
            <div class="options" v-if="optionsShow && filteredLocations.length > 0">
                <div class="option" v-for="skill in filteredLocations" :key="skill.id">
                    <input 
                        type="checkbox" 
                        class="input" 
...
watchEffect(() => {
    if (props.skillsCount && props.skillsCount.length > 0) {
        allSkills.value = [...props.skillsCount]
    }
})
...
const deleteLocationFromSelected = (skill) => {
    optionSelected.value = optionSelected.value.filter((e) => e.id !== skill.id)
    changeRoute()
}
...
const changeRoute = () =>{
  setTimeout(() => {
    ...
    const query = updateQueryParam("skills" , ids.join(','))
    router.push({
      name : 'talents',
      query
    })

    ...
    <section>
        ...
        <div class="container">
            <div class="main-page">
                    ...
                    <div class="filters">
                        ...
                        <div class="sort">
                            ...
                            <div class="options-box" v-if="showOptions" :style="{ display: showOptions ? 'flex' : 'none' }">
                                <div class="option" @click="selectOption('recommended' , 'sort' , 'Recommended')">
                                    <p>Recommended</p>
                                </div>
                                <div class="option" @click="selectOption('newest_arrivals' , 'sort' , 'Newest Arrivals')">
                                    <p>Newest Arrivals</p>
                                </div>
                                <div class="option" @click="selectOption('proposals' , 'sort' , 'Proposals')">
                                    <p>Proposals</p>
                                </div>
                                <div class="option" @click="selectOption('budget_asc' , 'sort' , 'Budget : Low To High')">
                                    <p>Budget : Low To High</p>
                                </div>
                                <div class="option" @click="selectOption('budget_desc' , 'sort' , 'Budget : High To Low')">
                                    <p>Budget : High To Low</p>
                                </div>
                            </div>
                        </div>
                ...
                <!-- Main Div Results -->
                <div class="main">
                    <JobsResults :filtersShow="filtersShow" :jobs="jobs"/>
                    <!-- Filter -- Right Side -->
                    <div class="right-side" v-if="filtersShow">
                        <RightSide :ratingCount="ratingCount" :locationCount="locationCount"/>
                    </div>
                </div>
            </div>
        </div>
...
import JobsResults from './LeftSide/JobsResults.vue'
...
const getCategories = async() => {
    try{
        const response = await axiosClient.get('/jobs/categories-count')
...
const getJobs = async(query) => {
    ...
    const budget_from = query['budgetRange'] ? query['budgetRange'].split(',')[0] : ''
    const budget_to = query['budgetRange'] ? query['budgetRange'].split(',')[1] : ''
    const form = {
        search : query.search || '',
        category_id : query.categories || '',
        sub_category_id : query.sub_categories || '',
        job_type : query.job_type || 'fast',
        budget_from : budget_from || '',
        budget_to : budget_to || '',
        allow_countries : query.allow_countries || '',
        sort_by : query.sort_by || 'recommended',
        per_page : query.per_page || 10,
        page : query.page || 1,
        client_rating : query.rating || '',
        duration : query.duration || 1 , // project length
        proposals : query.proposals || '',
        hires : query.hires || '' , // history
        payment_verified : query.payment_verified || 0,
    }
    ...
    try{
        const response = await axiosClient.get(`/jobs?
            ...
            allow_countries=${form.allow_countries}&
...
const getRatingCount = async() => {
    try{
        const response = await axiosClient.get('/jobs/rating-count')
...
const getLocationCount = async() => {
    try{
        const response = await axiosClient.get('/jobs/location-count')
...
const sortNameShow = ref('Recommended')
const showOptions = ref(false)
function optionsBoxToggle(){
    showOptions.value = !showOptions.value
}
function selectOption(option , type , optionNameShow){
    sortSellected.value = option
    sortNameShow.value = optionNameShow
    optionsBoxToggle()
    changeRoute(type)
}

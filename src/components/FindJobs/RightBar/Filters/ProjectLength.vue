<template>
    <div class="duration">
        ...
        <div class="checkbox" v-for="count of counts" :key="count.id" v-if="showStatus">
            <input type="checkbox" name="verified" v-model="typesSellected" :value="count.id" @change="changeRoute()"/>
            <p>{{count.value}}</p>
...
const changeRoute = () =>{
    const query = updateQueryParam("duration" , typesSellected.value.join(','))
    router.push({
      name : 'jobs',
      query
    })
...
onBeforeMount(async() => {
    try{
        const response = await axiosClient.get('/jobs/duration-count')
